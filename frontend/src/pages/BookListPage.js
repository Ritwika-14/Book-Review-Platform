...
const BookListPage = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 bg-slate-200">
      ...
      {/* Book Cards */}
      {loading && <p className="text-center text-lg font-medium mt-10 text-gray-600">Loading books...</p>}
      {error && <p className="text-center text-red-600 font-semibold mt-10">{error}</p>}
      ...
      <div className="grid gap-6 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {!loading && books.length > 0 ? (
          books.map((book) => (
            <div
              key={book._id}
              className="bg-white shadow-md hover:shadow-lg transition rounded-lg p-6 flex flex-col justify-between h-full hover:cursor-pointer"
            >
              <div>
              ...
              <div className="mt-4 flex flex-col items-center">
                ...
                <p className="text-xs text-gray-500 mt-1">({book.averageRating.toFixed(1)} / 5)</p>
                <Link
                  to={`/books/${book._id}`}
